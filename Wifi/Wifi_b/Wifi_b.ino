...
void setup() {
  ...
  //While it is connecting, print a '.' to the serial monitor every 500ms
...
void loop() {
  ...
  //Publish the sensor data tot the associated topics
  client.publish("4ID3_G7/temperature", String(dhtTempEvent.temperature).c_str());
  delay(100);
  client.publish("4ID3_G7/humidity", String(dhtHumEvent.relative_humidity).c_str());
  delay(100);
